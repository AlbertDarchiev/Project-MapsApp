...
//   return rootViewf
    ...
    override fun onMapLongClick(coord: LatLng) {
        mapViewModel.locationMap =
            Location("", coord.latitude.toString(), coord.longitude.toString(), "")
        findNavController().navigate(R.id.action_fragment_map_to_addLocationFragment)

    }
    fun updateMarks() {
        mapViewModel.listOfLocations = mutableListOf()
        db.collection("users").document(LoginFragment.emailLogged).collection("locations").get()
            .addOnSuccessListener { documents ->
                ...
                for (document in documents) {
                    loc = Location(
                        ...
                        document.get("imageFilename").toString()
                    )
                    ...
                    mapViewModel.listOfLocations.add(
                        Location(
                            loc.name, loc.latitude, loc.longitude, loc.image
                        )
                    )
                }
            }
        ...
        if (isLocationPermissionGranted()) {
            fusedLocationProviderClient.lastLocation.addOnCompleteListener {
                ...
                if (location != null) {
                    ...
                    map.animateCamera(
                        CameraUpdateFactory.newLatLngZoom(currentCoordinates, 18f), 5000, null
                    )
                }
            }
        } else {
